...
const tableName = 'CustomerScheduleAttachment';
const primaryKeyField = 'customerScheduleAttachmentId';
    ...
    DB.schema.dropTableIfExists(`${tableName}`).then(() => {
      DB.schema
        .createTable(`${tableName}`, function (table) {
          ...
          table.string('attachmentUrl', 500);
          timestamps(table);
...
function _makeQueryBuilderByFilter(filter, skip, limit, startDate, endDate, searchText, order) {
  ...
  if (searchText) {
    queryBuilder.where(function () {
      this.orWhere('attachmentName', 'like', `%${searchText}%`);
    });
  }
