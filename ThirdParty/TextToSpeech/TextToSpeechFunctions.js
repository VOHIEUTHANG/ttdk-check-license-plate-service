...
async function getPlateSpeechUrls(plateNumber) {
  const speeches = [];
  let characters = plateNumber.split('');

  //Add welcome speech
  speeches.push(`https://${process.env.HOST_NAME}/uploads/voices/moi-chu-xe.mp3`);
  ...
  for (let i = charCounter; i < characters.length; i++) {
    ...
    vehicleNumber += charString + '!';
  }
  let vehicleSpeechfile = await TextToSpeechClient.makeSpeechFile(vehicleNumber, `uploads/voices/${vehicleNumber}.mp3`);
  if (vehicleSpeechfile) {
    speeches.push(`https://${process.env.HOST_NAME}/uploads/voices/${vehicleNumber}.mp3`);
  } else {
    console.error(`failed to getPlateSpeechUrls for vehicleNumber ${vehicleNumber}`);
    return undefined;
  }
